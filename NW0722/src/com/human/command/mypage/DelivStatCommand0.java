...
public class DelivStatCommand0 implements ICommand {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		System.out.println("@@@@@@@@@@@@ DelivStatCommand0  @@@@@@@@@@@@@@ periodSearch = "+period+" @@@@@@@@@@@");
...
//		ArrayList<ProductsDto> pDtos = ProductsDao.select();
		...
		if (period != null) {
			if (period.equals("0")) {
				orderStat = dao.ordersOrderdetailsProductsJoin_0(oidList, "orders");
				productStat = dao.ordersOrderdetailsProductsJoin_0(oidList, "products");
				detailStat = dao.ordersOrderdetailsProductsJoin_0(oidList, "orderDetails");
			} else {
				orderStat = dao.ordersOrderdetailsProductsJoin_0(oidList, "orders", period);
				productStat = dao.ordersOrderdetailsProductsJoin_0(oidList, "products", period);
				detailStat = dao.ordersOrderdetailsProductsJoin_0(oidList, "orderDetails", period);
			}
