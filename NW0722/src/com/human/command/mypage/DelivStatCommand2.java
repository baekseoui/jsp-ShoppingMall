...
public class DelivStatCommand2 implements ICommand {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		System.out.println("@@@@@@@@@@@@ DelivStatCommand2  @@@@@@@@@@@@@@ periodSearch = "+period+" @@@@@@@@@@@");
...
//		ArrayList<ProductsDto> pDtos = ProductsDao.select();
		...
		if (period != null) {
			if (period.equals("0")) {
				orderStat = dao.ordersOrderdetailsProductsJoin_2(oidList, "orders");
				productStat = dao.ordersOrderdetailsProductsJoin_2(oidList, "products");
				detailStat = dao.ordersOrderdetailsProductsJoin_2(oidList, "orderDetails");
			} else {
				orderStat = dao.ordersOrderdetailsProductsJoin_2(oidList, "orders", period);
				productStat = dao.ordersOrderdetailsProductsJoin_2(oidList, "products", period);
				detailStat = dao.ordersOrderdetailsProductsJoin_2(oidList, "orderDetails", period);
			}
